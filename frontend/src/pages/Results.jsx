...
// Map rule IDs / fields to bbox fields for highlighting
const RULE_FIELD_MAP = {
  LM_001: 'manufacturer',
  LM_002: 'country_of_origin',
  LM_003: null,
  LM_004: 'quantity',
  LM_005: 'dates',
  LM_006: 'dates',
  LM_007: 'mrp',
  LM_008: 'consumer_care',
  LM_009: 'consumer_care',
};

function resolveFieldFromRule(check) {
  if (!check) return null;
  // Try rule_id first (e.g., "LM-001" or "LM_001")
  const normalized = (check.rule_id || '').replace('-', '_');
  if (RULE_FIELD_MAP[normalized]) return RULE_FIELD_MAP[normalized];
  // Fallback to the `field` on the check
  const f = (check.field || '').toLowerCase();
  if (f.includes('manufacturer') || f.includes('packer') || f.includes('importer')) return 'manufacturer';
  if (f.includes('quantity')) return 'quantity';
  if (f.includes('mrp') || f.includes('price')) return 'mrp';
  if (f.includes('date') || f.includes('best_before') || f.includes('packing')) return 'dates';
  if (f.includes('consumer')) return 'consumer_care';
  if (f.includes('origin')) return 'country_of_origin';
  return null;
}
...
export default function Results({
  ...
  const handleOpenCheckModal = (chk) => {
    ...
    setIsModalOpen(true);
    const field = resolveFieldFromRule(chk);
  ...
  const handleBoxClick = (field) => {
    // Toggle highlight if the same field is clicked
    setHighlightedField((prev) => (prev === field ? null : field));
  };

  // Fallback demo image if product.image_url missing
  const fallbackImage =
    product.image_url ||
    (is_demo
      ? product.product_name?.includes('Butter')
        ? 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="600" height="400"><rect width="600" height="400" fill="%230F2942"/><text x="50" y="80" fill="%23FFF" font-size="28">ROYAL TREATS</text><text x="50" y="130" fill="%23FFF" font-size="20">Butter Delight Biscuits</text><text x="50" y="200" fill="%23FCD34D" font-size="16">Net Wt: 200g | MRP: Rs. 80.00</text><text x="50" y="250" fill="%23FFF" font-size="14">Mfg: 07/2026 | Best Before: 6 mos</text><text x="50" y="300" fill="%23CBD5E1" font-size="12">ABC Foods, Hyderabad - 500076</text></svg>'
    ...
    <div className="space-y-6 pb-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="p-4 rounded-xl border border-emerald-200 bg-emerald-50/50 shadow-2xs flex flex-col justify-between">
            ...
            <div className="text-[10px] text-emerald-700/80 mt-1 font-medium">Rules Verified</div>
          </div>
          ...
          <div className="p-4 rounded-xl border border-rose-200 bg-rose-50/50 shadow-2xs flex flex-col justify-between">
            ...
            <div className="text-[10px] text-rose-700/80 mt-1 font-medium">Definite Violations</div>
          </div>
          ...
          <div className="p-4 rounded-xl border border-amber-200 bg-amber-50/50 shadow-2xs flex flex-col justify-between">
            ...
            <div className="text-[10px] text-amber-700/80 mt-1 font-medium">Needs Officer Check</div>
          </div>
          ...
          <div className="p-4 rounded-xl border border-slate-200 bg-slate-50/80 shadow-2xs flex flex-col justify-between">
            ...
            <div className="text-[10px] text-slate-500 mt-1 font-medium">Not Applicable</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 border-b border-slate-200 pb-3">
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-colors ${
            activeView === 'checks' ? 'bg-[#0F2942] text-white shadow-xs' : 'text-slate-600 hover:bg-slate-200/60'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-colors ${
            activeView === 'extracted' ? 'bg-[#0F2942] text-white shadow-xs' : 'text-slate-600 hover:bg-slate-200/60'
          }`}
        >
      ...
      {activeView === 'checks' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          {/* Left Column: Image Preview with bbox overlay */}
          <div className="lg:col-span-4 space-y-4">
            <ImagePreviewCard
              imageSrc={fallbackImage}
              fileInfo={{ name: 'uploaded_label.jpg' }}
            ...
            {highlightedField && (
              <div className="text-[11px] text-slate-500 px-2">
                Highlighting: <strong className="text-slate-700">{highlightedField.replace('_', ' ')}</strong>
                <button
          ...
          <div className="lg:col-span-8 space-y-3">
            <div className="flex items-center justify-between px-1">
              ...
              <span className="text-xs text-slate-500">
                Click any check to highlight the declaration on the label
              </span>
            </div>
      ...
      {/* Tab 2: Extracted Info */}
      {activeView === 'extracted' && (
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs">
          <div className="mb-4">
            <h3 className="text-base font-bold text-slate-900">Extracted Package Declarations</h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Structured representation of declarations identified by AI on visible label surfaces.
            </p>
          </div>
      ...
      <div className="p-4 rounded-xl bg-slate-100 border border-slate-200 text-xs text-slate-500 flex items-start gap-2.5">
        ...
        <p className="leading-relaxed">
          <strong>Legal Disclaimer:</strong> Prototype screening result. Final regulatory determination should be verified by an authorized Legal Metrology officer.
        </p>
      </div>
